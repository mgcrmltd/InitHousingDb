...
namespace SetupHousingDB
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var gd = new GenerateData(housingContext);
            Console.ReadKey();
        }
        ...
        public class GenerateData
        {
            ...
            public void CreateEstates(int count, int idSeed)
            {
                var propType = _context.PropertyTypes.ToList().First(x => x.Name == "EST");
                for (var i = 0; i < count; i++)
                {
                    var id = idSeed + _properties.Count;
                    _properties.Add(GetProperty(propType,null, null,id));
                }
            }

            public void CreateBlocks(int idSeed) //Same address as est
            {
                var propType = _context.PropertyTypes.ToList().First(x => x.Name == "BLK");
                var estates = _properties.Where(x => x.PropertyTypeId.Name == "EST");

                foreach (var estate in estates)
                {
                    var id = idSeed + _properties.Count;
                    _properties.Add(GetProperty(propType,null, null,id, estate));
                }
            }

            public void CreateFlats(int idSeed) //Should have same address as block plus secondary
            {
                var propType = _context.PropertyTypes.ToList().First(x => x.Name == "FLT");
                var blocks = _properties.Where(x => x.PropertyTypeId.Name == "BLK");

                foreach (var block in blocks)
                {
                    var id = idSeed + _properties.Count;
                    _properties.Add(GetProperty(propType,null, null,id, block));
                }
            }


            public Property GetProperty(PropertyType type, PropertySubType subType, LeaseType leaseType, int id, Property parent = null)
            {
                return new Property()
                    {Id = id, PropertyTypeId = type, PropertySubTypeId = subType, LeaseTypeId = leaseType, ParentId = parent};
            }
